...
void CardReader::fileHasFinished() {
  ...
  if (file_subcall_ctr > 0) { // Resume calling file after closing procedure
    ...
    marlin_state = MF_SD_COMPLETE;
      ...
      //MARIO: it should not be necessary because is done in other Marlin's methods
      // #if ENABLED(POWER_LOSS_RECOVERY)
      //     card.openJobRecoveryFile(false);
      //     recovery.info.valid_head = 0;
      //     recovery.info.valid_foot = 0;
      //     recovery.save(); //MARIO: in Longer code was .write(), but now in Marlin 2 write() is private
      //     recovery.file.close();
      // #endif
    #endif // LGT_MAC
  }
