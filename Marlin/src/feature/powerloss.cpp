...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0*/) {
  ...
  if (force
    ...
    info.have_percentdone = card.percentDone();
...
void PrintJobRecovery::resume() {
  //MARIO: original Longer Marlin 1.1.9 code. Should be used? NB: This code could be also in check()
  ...
  //   //MARIO: maybe we should put zraise = 0
  //   dtostrf(PrintJobRecovery::info.current_position.z, 1, 3, str_Z);
	// 	dtostrf(PrintJobRecovery::info.current_position.e, 1, 3, str_E); 
  ...
  //MARIO: maybe HAS_LEVELING should be sobstituted with the U20 code which should be used also for U30 Pro with ABL
  #if HAS_LEVELING
    // Make sure leveling is off before any G92 and G28
    gcode.process_subcommands_now_P(PSTR("M420 S0 Z0"));
  #endif
  //MARIO: maybe to enable. This code should be reviewed
  // #ifdef U20_Pro
