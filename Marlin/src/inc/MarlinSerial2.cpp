...
/**
 * MarlinSerial2.cpp - Hardware serial library for Wiring
...
// #include "emergency_parser.h"
  ...
  void MarlinSerial2::begin(const long baud) {
  ...
  void MarlinSerial2::checkRx(void) {
  ...
  ring_buffer_pos_t1 MarlinSerial2::available(void) {
    CRITICAL_SECTION_START();
  ...
  void MarlinSerial2::flush(void) {
  ...
  #if TX_BUFFER_SIZE1 > 0
    uint8_t MarlinSerial2::availableForWrite(void) {
      CRITICAL_SECTION_START();
    ...
    void MarlinSerial2::write(const uint8_t c) {
      #if ENABLED(SERIAL_XON_XOFF)
        const uint8_t state1 = xon_xoff_state1;
        if (!(state1 & XON_XOFF_CHAR_SENT1)) {
    ...
    void MarlinSerial2::writeNoHandshake(const uint8_t c) {
      _written1 = true;
      CRITICAL_SECTION_START();
    ...
    void MarlinSerial2::flushTX(void) {
      // TX
    ...
    void MarlinSerial2::writeNoHandshake(uint8_t c) {
      while (!TEST(M_UCSRxA1, M_UDREx1)) {/* nada */}
      M_UDRx1 = c;    
    }
  ...
  // Preinstantiate
 MarlinSerial2 customizedSerial3;
