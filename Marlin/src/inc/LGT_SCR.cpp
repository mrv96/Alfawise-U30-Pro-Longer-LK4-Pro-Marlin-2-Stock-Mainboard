//MARIO: vedere se si imposta correttamente l'offset dall scermata di autolivellamento (sistemare eventualmente level_z_height) (attenzione alla riga 1448)
//MARIO: offset setting from LCD should be tried. If any problem adjust level_z_height. (maybe row 1448 should be adjusted)
...
char leveling_sta = 0;
float level_z_height = INITIAL_LEVEL_Z_HEIGHT;
...
void LGT_SCR::LGT_Change_Page(unsigned int pageid)
{
	...
	for (int i = 0; i < 10; i++)
		MYSERIAL2.print(data_storage[i]);
}
...
void LGT_SCR::LGT_MAC_Send_Filename(uint16_t Addr, uint16_t Serial_Num)
{
	...
	data_storage[4] = (Addr & 0xFF00) >> 8;
	data_storage[5] = Addr;
	card.selectFileByIndex(Serial_Num);
	for (int i = 0; i < 27; i++)
	{
...
**************************************/
void LGT_SCR::LGT_Get_MYSERIAL2_Cmd()
{
	memset(data_storage, 0, sizeof(data_storage));
	while (re_count<DATA_SIZE &&MYSERIAL2.available() > 0)
	{
		data_storage[re_count] = MYSERIAL2.read();
...
{
	//MARIO: Old Longer Marlin 1.1.9 code
// 		card.endFilePrint(
// #if SD_RESORT
// 			true
// #endif
...
// 		TERN_(POWER_LOSS_RECOVERY, recovery.purge()); //MARIO: messo da me in sostituzione di sotto
...
// 	// #endif
	...
	//MARIO: commented out because now are in abortSDPrinting() (MarlinCore.cpp)
...
//			}
		...
		case eBT_MOVE_Z_PLUS_0:
			if (current_position[Z_AXIS] < Z_MAX_POS) {
				...
				{
					if(level_z_height == INITIAL_LEVEL_Z_HEIGHT) level_z_height = probe.offset.z + 10;
					level_z_height = 10 + level_z_height;
					LGT_Send_Data_To_Screen(ADDR_VAL_LEVEL_Z_UP_DOWN, (uint16_t)(100 * level_z_height));
				}
#endif // U20_Pro
			}
			break;
...
//			}
		...
		case eBT_MOVE_E_PLUS_0:
			if (thermalManager.temp_hotend[0].celsius >= LGT_Get_Extrude_Temp())
			{
		...
		case eBT_MOVE_Z_HOME:
			delay(5);
				...
				xy_home = true;
                 ...
                 #endif
				 queue.enqueue_one_P(PSTR("G28 Z0")); //U30_Pro
				z_home = true;
			#endif
			break;
...
//			queue.inject_P(PSTR("M84"));
		...
		case eBT_PRINT_HOME_ABORT:
				LGT_Change_Page(ID_DIALOG_PRINT_WAIT);
				wait_for_heatup = false;
				LGT_stop_printing = true; // MARIO: In future remove this and use only card.flag.abort_sd_printing = true;
		...
		case eBT_UTILI_FILA_PLA:
			...
			LGT_Send_Data_To_Screen(ADDR_VAL_TAR_E, thermalManager.temp_hotend[0].celsius);
			delayMicroseconds(1);
			LGT_Send_Data_To_Screen(ADDR_VAL_TAR_B, thermalManager.temp_bed.celsius);
			LGT_Send_Data_To_Screen(ADDR_VAL_FILA_CHANGE_TEMP, thermalManager.temp_hotend[0].celsius);
			break;
		...
		case eBT_UTILI_FILA_LOAD:
				if (thermalManager.temp_hotend[0].celsius >= (filament_temp - 5))
				{
...
//////////////////////////////////////////////////////////////////////////
		...
		case eBT_UTILI_LEVEL_CORNER_POS_2: //45 002D
				...
				queue.enqueue_one_P(PSTR("G1 Z10"));
				queue.enqueue_one_P(PSTR("G1 X199 Y30 F3000"));
				queue.enqueue_one_P(PSTR("G1 Z0"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_3:
				...
				queue.enqueue_one_P(PSTR("G1 Z10"));
				queue.enqueue_one_P(PSTR("G1 X199 Y205 F3000"));
				queue.enqueue_one_P(PSTR("G1 Z0"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_4:
				...
				queue.enqueue_one_P(PSTR("G1 Z10"));
				queue.enqueue_one_P(PSTR("G1 X30 Y205 F3000"));
				queue.enqueue_one_P(PSTR("G1 Z0"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_5:
				...
				queue.enqueue_one_P(PSTR("G1 Z10"));
				queue.enqueue_one_P(PSTR("G1 X114.5 Y117.5 F3000"));
				queue.enqueue_one_P(PSTR("G1 Z0"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_BACK:
				...
				if (xy_home) {
					xy_home = false;
					queue.inject_P(PSTR("G1 Z10"));	//up 10mm to prevent from damaging bed
				}
			#else
				if (xyz_home) {
					xyz_home = false;
      ...
      thermalManager.setTargetHotend(0, target_extruder);
      thermalManager.setTargetBed(0);
			//level_z_height = probe.offset.z + 10; //use level_z_height to show onscreen zoffset
			#if defined(U30_Pro_AutoBed)
        //Perform hot leveling
        queue.inject_P(PSTR("M104 S210"));
        queue.enqueue_one_P(PSTR("M190 S65"));
        //Finish heat up waiting
				...
				queue.enqueue_one_P(PSTR("G1 Z10"));
				queue.enqueue_one_P(PSTR("G1 X114.25 Y115 F3000"));
				queue.enqueue_one_P(PSTR("G1 Z0"));
        queue.enqueue_one_P(PSTR("M2002"));
			#else
        queue.inject_P(PSTR("G28 X0 Y0"));
        queue.enqueue_one_P(PSTR("G1 X180 Y153 F3000"));
        queue.enqueue_one_P(PSTR("M2002"));
        xy_home = true;
			#endif
			break;
		...
		case eBT_UTILI_LEVEL_MEASU_S2_NEXT:
			...
			#if defined(U30_Pro_AutoBed)
        //settings.reset();
		if(level_z_height != INITIAL_LEVEL_Z_HEIGHT) probe.offset.z = level_z_height; //save the new Z Offset
        //Start UBL, enable it and save to EEPROM
				leveling_sta = 1;  //set the lvelling to ok to show the levelling finished on G29 call
        //queue.inject_P(PSTR("G28")); //Autohome (probably not needed)
        queue.inject_P(PSTR("G29 P1")); //invalidate the mesh and full probe the bed
        queue.enqueue_one_P(PSTR("G29 P3")); //fill unprobed points
        queue.enqueue_one_P(PSTR("G29 P5 C")); //set mean mesh to a 0 centered (avoid mesh shifted up by some mm)
        queue.enqueue_one_P(PSTR("G29 S1")); //save the mesh
        queue.enqueue_one_P(PSTR("G29 A")); //activate UBL
        queue.enqueue_one_P(PSTR("M500")); //save to EEPROM
        queue.enqueue_one_P(PSTR("G29")); //workaround to just to show the levelling finished on LCD
      #else
        settings.reset();
        queue.inject_P(PSTR("G28"));
		...
		case eBT_UTILI_LEVEL_MEASU_STOP_MOVE:
			LGT_Send_Data_To_Screen(ADDR_VAL_LEVEL_Z_UP_DOWN, /*zprobe_zoffset + 10*/ level_z_height);
			queue.clear();
			quickstop_stepper();
...
////////////////////////////////////////////////////////////////////////
		...
		break;
	case ADDR_VAL_FAN:
		thermalManager.fan_speed[0] = Rec_Data.data[0];
		break;
	case ADDR_VAL_FEED:
	...
	if (millis() >= Next_Temp_Time)
	{
		...
		if (tartemp_flag == true)  //M104/M109/M190/140
		{
			...
			if(LGT_is_printing==false)
				status_type = PRINTER_HEAT;
			LGT_Send_Data_To_Screen(ADDR_VAL_TAR_E, thermalManager.temp_hotend[0].target);
			LGT_Send_Data_To_Screen(ADDR_VAL_TAR_B, thermalManager.temp_bed.target);
		}
		LGT_Printer_Data_Updata();
		LGT_Get_MYSERIAL2_Cmd();
	}
...
void LGT_SCR::LGT_Printer_Data_Updata()
{
	...
	{
	case eMENU_HOME:
		LGT_Send_Data_To_Screen(ADDR_VAL_CUR_E, (int16_t)thermalManager.temp_hotend[0].celsius);
		LGT_Send_Data_To_Screen(ADDR_VAL_CUR_B, (int16_t)thermalManager.temp_bed.celsius);
		break;
	case eMENU_TUNE:
		LGT_Send_Data_To_Screen(ADDR_VAL_CUR_E, (int16_t)thermalManager.temp_hotend[0].celsius);
		LGT_Send_Data_To_Screen(ADDR_VAL_CUR_B, (int16_t)thermalManager.temp_bed.celsius);
		LGT_Get_MYSERIAL2_Cmd();
		LGT_Send_Data_To_Screen(ADDR_VAL_FAN,thermalManager.fan_speed[0]);
...
{
	if (LGT_is_printing == false&&((thermalManager.temp_bed.celsius < thermalManager.temp_bed.target)
		|| (thermalManager.temp_hotend[0].celsius < (TEMP_RANGE+thermalManager.temp_hotend[0].target))))
	{
		status_type = PRINTER_HEAT;
	}
	else if (LGT_is_printing == false&&((thermalManager.temp_bed.celsius >=thermalManager.temp_bed.target)
		|| (thermalManager.temp_hotend[0].celsius > (thermalManager.temp_hotend[0].target))
		))
	{
		status_type = PRINTER_STANDBY;
	}
...
void LGT_SCR::LGT_Power_Loss_Recovery_Resume() {
	//MARIO: old code sobstituted by queue.inject_P(PSTR("M1000"))
// 	char cmd[30];
// 	// Restore all hotend temperatures
// 	sprintf_P(cmd, PSTR("M190 S%i"), recovery.info.target_temperature_bed);
// 	queue.inject(cmd);
// 	sprintf_P(cmd, PSTR("M109 S%i"), recovery.info.target_temperature[0]);
// 	queue.inject(cmd);
// 	// Restore print cooling fan speeds
// 	for (uint8_t i = 0; i < FAN_COUNT; i++) {
// 		int16_t f = recovery.info.fan_speed[i];
// 		if (f) {
...
// 	//recovery.enable(true); // OLD: job_recovery_phase = JOB_RECOVERY_YES;

	recovery_time = recovery.info.print_job_elapsed;
	recovery_percent = recovery.info.have_percentdone;
	recovery_z_height = recovery.info.current_position.z; // OLD: recovery_z_height = recovery.info.save_current_Z;
